...
class ApiPolicyStudioController {
  ...
  fetchResourceDocumentation(event) {
    this.studio.setAttribute('documentation', null);
    const {detail: {resourceType}} = event;
    event.originalTarget.setAttribute('documentation', null);
    this.ResourceService.getDocumentation(resourceType.id).then((response) => {
      event.originalTarget.setAttribute('documentation', JSON.stringify({content: response.data, image: resourceType.icon}));
    });
  }
